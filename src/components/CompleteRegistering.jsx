...
import { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { completeRegister, resetUserState } from "../redux/auth/authSlice";
import {
  Box,
  Grid,
  TextField,
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField
              name="firstName"
              label={t("completeRegister.firstName")}
              fullWidth
              required
              variant="outlined"
              value={formData.firstName}
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              name="lastName"
              label={t("completeRegister.lastName")}
              fullWidth
              required
              variant="outlined"
              value={formData.lastName}
          ...
          <Grid item xs={12}>
            <TextField
              name="national_number"
              label={t("completeRegister.nationalNumber")}
              fullWidth
              required
              variant="outlined"
              value={formData.national_number}
          ...
          <Grid item xs={12}>
            <TextField
              name="password"
              label={t("completeRegister.password")}
              type="password"
              fullWidth
              required
              variant="outlined"
          ...
          <Grid item xs={12}>
            <Button
              ...
              sx={{ py: 1.5 }}
            >
              {image ? image.name : t("completeRegister.uploadImage")}
              <input
                type="file"
                hidden
                accept="image/*"
          ...
          <Grid item xs={12}>
            ...
            >
              {loading ? (
                ...
                t("completeRegister.submitButton")
              )}
            </Button>
          </Grid>
          {error && (
            <Grid item xs={12}>
              <Alert severity="error">
                {t("completeRegister.errors.somethingWentWrong")}
              </Alert>
            </Grid>
          )}
          {success && (
            <Grid item xs={12}>
              <Alert severity="success">
                {t("completeRegister.messages.success")}
              </Alert>
            </Grid>
          )}
        </Grid>
