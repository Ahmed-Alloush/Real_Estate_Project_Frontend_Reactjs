...
const UpdatePropertyPage = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      await dispatch(updateProperty({ id, updatedData })).unwrap();
      Swal.fire(t("updateProperty.success"), t("updateProperty.successMessage"), "success");
      navigate(-1);
    } catch (error) {
      Swal.fire(
        t("updateProperty.error"),
        `${t("updateProperty.errorMessage")}: ${error.message || t("updateProperty.unknownError")}`,
        "error"
      );
    }
  };
  ...
  const handlePhotoAdd = async () => {
    if (!newPhotoFile) {
      Swal.fire(t("updateProperty.warning"), t("updateProperty.selectPhotoWarning"), "warning");
      return;
    }
    ...
    try {
      ...
      Swal.fire(t("updateProperty.success"), t("updateProperty.photoAddedSuccess"), "success");
      setNewPhotoFile(null);
    } catch (error) {
      Swal.fire(
        t("updateProperty.error"),
        `${t("updateProperty.failedToAddPhoto")}: ${error.message || t("updateProperty.unknownError")}`,
        "error"
      );
    }
  };
  ...
  const handlePhotoRemove = async (photoId) => {
    try {
      await dispatch(removePhotoFromProperty(photoId)).unwrap();
      Swal.fire(t("updateProperty.success"), t("updateProperty.photoDeletedSuccess"), "success");
    } catch (error) {
      Swal.fire(
        t("updateProperty.error"),
        `${t("updateProperty.failedToDeletePhoto")}: ${error.message || t("updateProperty.unknownError")}`,
        "error"
      );
    }
  };
  ...
  const handleAttributeAdd = async () => {
    if (!newAttributeName || !newAttributeValue) {
      Swal.fire(
        t("updateProperty.warning"),
        t("updateProperty.fillAttributesWarning"),
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      Swal.fire(t("updateProperty.success"), t("updateProperty.attributeAddedSuccess"), "success");
      setNewAttributeName("");
      setNewAttributeValue("");
    } catch (error) {
      Swal.fire(
        t("updateProperty.error"),
        `${t("updateProperty.failedToAddAttribute")}: ${error.message || t("updateProperty.unknownError")}`,
        "error"
      );
    }
  };
  ...
  const handleAttributeRemove = async (attributeId) => {
    try {
      await dispatch(removeAttributeFromProperty(attributeId)).unwrap();
      Swal.fire(t("updateProperty.success"), t("updateProperty.attributeDeletedSuccess"), "success");
    } catch (error) {
      Swal.fire(
        t("updateProperty.error"),
        `${t("updateProperty.failedToDeleteAttribute")}: ${error.message || t("updateProperty.unknownError")}`,
        "error"
      );
    }
  };
  ...
  if (currentPropertyError) {
    return (
      <Box textAlign="center" mt={4}>
        <Typography color="error">
          {t("updateProperty.error")}: {currentPropertyError}
        </Typography>
      </Box>
    );
  }
  ...
  if (!currentProperty) {
    return (
      <Box textAlign="center" mt={4}>
        <Typography>{t("updateProperty.propertyNotFound")}</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box p={4} sx={{ direction: isRTL ? "rtl" : "ltr" }}>
      <Button variant="contained" color="primary" sx={{ mt: 3 }} onClick={() => navigate(-1)}>
        {t("updateProperty.goBackToOffice")}
      </Button>
      <Paper elevation={3} sx={{ p: 4, mt: 3 }}>
        <Typography variant="h4" gutterBottom>
          {t("updateProperty.updateTitle", { number: currentProperty.propertyNumber })}
        </Typography>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <Box mb={3}>
              <Typography variant="h6" mb={2}>
                {t("updateProperty.propertyDetails")}
              </Typography>
              <TextField
                fullWidth
                label={t("updateProperty.price")}
                type="number"
                value={price}
                onChange={(e) => setPrice(e.target.value)}
                sx={{ mb: 2 }}
              ...
              <TextField
                fullWidth
                label={t("updateProperty.description")}
                multiline
                rows={4}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
              ...
              />
              <FormControl fullWidth>
                <InputLabel>{t("updateProperty.typeOperation")}</InputLabel>
                <Select
                  value={propertyTypeOperation}
                  label={t("updateProperty.typeOperation")}
                  onChange={(e) => setPropertyTypeOperation(e.target.value)}
                >
                  <MenuItem value="selling">{t("updateProperty.selling")}</MenuItem>
                  <MenuItem value="renting">{t("updateProperty.renting")}</MenuItem>
                </Select>
              </FormControl>
              <Button
                variant="contained"
            ...
            <Box mb={3}>
                  ...
                  <Grid item key={photo.id} xs={6} sm={4} md={3}>
                    <Box sx={{ position: "relative" }}>
                      <Box
                        component="img"
                        src={photo.url}
                        alt={t("updateProperty.propertyPhotoAlt")}
                        sx={{
                          width: "100%",
                          height: 100,
                          objectFit: "cover",
              ...
              <Box mt={3} display="flex" alignItems="center" gap={2}>
                <Button
                  ...
                  sx={{ minWidth: 200 }}
                >
                  {t("updateProperty.addPhotoButton")}
                  <input
                    type="file"
                    hidden
                    accept="image/*"
          ...
          <Grid item xs={12}>
            <Box>
              ...
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={12} sm={5}>
                  <FormControl fullWidth>
                    <InputLabel>{t("updateProperty.attributeName")}</InputLabel>
                    <Select
                      value={newAttributeName}
                      label={t("updateProperty.attributeName")}
                      onChange={(e) => setNewAttributeName(e.target.value)}
                    >
                      {allowedAttributes.map((allowedAttr) => (
                        <MenuItem key={allowedAttr.id} value={allowedAttr.name}>
                ...
                <Grid item xs={12} sm={5}>
                  <TextField
                    fullWidth
                    type="number"
                    label={t("updateProperty.attributeValue")}
                    value={newAttributeValue}
                    onChange={(e) => setNewAttributeValue(e.target.value)}
                  />
                </Grid>
