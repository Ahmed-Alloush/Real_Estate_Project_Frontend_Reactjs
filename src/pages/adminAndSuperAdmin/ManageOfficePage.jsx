...
function ManageOfficesPage() {
  const dispatch = useDispatch();
  const { t, i18n } = useTranslation(); // Use the hook to get t and i18n
  const { offices, loading, error, statusUpdateLoading, pagination } =
    useSelector((state) => state.manageOffice);
  ...
  // Adjust direction based on current language (for RTL support)
  useEffect(() => {
  ...
  const handleApprove = (officeId) => {
    Swal.fire({
      title: t("manageOffices.approveTitle"),
      text: t("manageOffices.approveText"),
      icon: "question",
      showCancelButton: true,
      confirmButtonColor: "#28a745",
      cancelButtonColor: "#6c757d",
      confirmButtonText: t("manageOffices.approveConfirmBtn"),
    }).then((result) => {
      if (result.isConfirmed) {
        dispatch(updateOfficeStatus({ officeId, status: "accepted" }));
        Swal.fire(
          t("manageOffices.approveSuccessTitle"),
          t("manageOffices.approveSuccessText"),
          "success"
        );
      }
    });
  ...
  const handleReject = (officeId) => {
    Swal.fire({
      title: t("manageOffices.rejectTitle"),
      text: t("manageOffices.rejectText"),
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#dc3545",
      cancelButtonColor: "#6c757d",
      confirmButtonText: t("manageOffices.rejectConfirmBtn"),
    }).then((result) => {
      if (result.isConfirmed) {
        dispatch(updateOfficeStatus({ officeId, status: "rejected" }));
        Swal.fire(
          t("manageOffices.rejectSuccessTitle"),
          t("manageOffices.rejectSuccessText"),
          "info"
        );
      }
    });
  ...
  const handleDelete = (officeId) => {
    Swal.fire({
      title: t("manageOffices.deleteTitle"),
      text: t("manageOffices.deleteText"),
      icon: "error",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      cancelButtonColor: "#3085d6",
      confirmButtonText: t("manageOffices.deleteConfirmBtn"),
    }).then((result) => {
      if (result.isConfirmed) {
        dispatch(deleteOffice(officeId));
        Swal.fire(
          t("manageOffices.deleteSuccessTitle"),
          t("manageOffices.deleteSuccessText"),
          "success"
        );
      }
    });
    ...
    <Container sx={{ mt: 4 }}>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {error.message ||
            error.response?.data?.message ||
            t("manageOffices.error")}
        </Alert>
      )}

      {statusUpdateLoading && (
        <Alert severity="info" sx={{ mb: 2 }}>
          {t("manageOffices.updateStatus")}
        </Alert>
      )}

      {!loading && offices.length === 0 && (
        <Alert severity="info">{t("manageOffices.noPendingOffices")}</Alert>
      )}
          ...
          <Grid item xs={12} sm={6} md={4} key={office.id}>
            ...
            >
              ...
              <CardContent sx={{ flexGrow: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  <strong>{t("manageOffices.status")} :</strong>{" "}
                  {office.status ? office.status.toUpperCase() : "N/A"}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  <strong>{t("manageOffices.email")} :</strong>{" "}
                  {office.office_email}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  <strong>{t("manageOffices.phone")} :</strong>{" "}
                  {office.office_phone}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  <strong>{t("manageOffices.manager")} :</strong>{" "}
                  {office.user?.first_name} {office.user?.last_name}
                </Typography>
              </CardContent>
              <CardActions
                sx={{
