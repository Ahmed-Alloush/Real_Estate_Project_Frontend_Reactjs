...
function FavoritePropertyPage() {
  ...
  const handleClearAllFavorites = () => {
    if (window.confirm(t("favoriteProperties.confirmRemoveAll"))) {
      dispatch(deleteAllFavoriteProperties());
    }
  };

  const handleDeleteFavoriteProperty = (propertyId) => {
    if (window.confirm(t("favoriteProperties.confirmRemoveOne"))) {
      dispatch(deleteFavoriteProperty(propertyId));
    }
  };
  ...
  if (favoritePropertiesLoading) {
    return (
      <Container sx={{ mt: 6, textAlign: "center" }}>
        <CircularProgress />
        <Typography mt={2}>{t("favoriteProperties.loading")}</Typography>
      </Container>
    );
  }
  ...
  return (
    <Container sx={{ mt: 4, pb: 6 }}>
      ...
      {favoritePropertiesError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {t("favoriteProperties.errorLoading")}:{" "}
          {favoritePropertiesError.message || t("favoriteProperties.unknownError")}
        </Alert>
      )}

      {favoriteActionError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {t("favoriteProperties.errorAction")}:{" "}
          {favoriteActionError.message || t("favoriteProperties.unknownError")}
        </Alert>
      )}

      {favoriteProperties.length === 0 ? (
        <Alert severity="info" sx={{ mt: 3 }}>
          {t("favoriteProperties.noFavorites")}
        </Alert>
      ) : (
        <Grid container spacing={4} sx={{ mt: 2 }}>
          {favoriteProperties.map((favoriteItem) => {
            ...
            return (
              <Grid item key={property.id} xs={12} sm={6} md={4}>
                <Card sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
                  <Box sx={{ position: "relative" }}>
                    ...
                    >
                      <CardMedia
                        ...
                        height="200"
                        image={getFirstImageUrl(property)}
                        alt={t("favoriteProperties.imageAlt", {
                          number: property.propertyNumber,
                        })}
                      />
                    </CardActionArea>
                    <IconButton
                      aria-label={t("favoriteProperties.removeAria")}
                      onClick={() => handleDeleteFavoriteProperty(property.id)}
                      disabled={favoriteActionLoading}
                      sx={{
                        position: "absolute",
                  ...
                  <CardContent sx={{ flexGrow: 1 }}>
                    <Typography variant="h6" fontWeight="bold" noWrap>
                      {t("favoriteProperties.propertyNumber", {
                        number: property.propertyNumber,
                      })}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                      {t("favoriteProperties.operation")}: {property.typeOperation}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {t("favoriteProperties.space")}: {property.space} m²
                    </Typography>
                    <Typography
                      variant="body1"
                      color="primary"
                      ...
                      sx={{ mt: 1 }}
                    >
                      {t("favoriteProperties.price")}: {property.price}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
