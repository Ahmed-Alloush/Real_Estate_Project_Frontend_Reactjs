...
const FavoriteOfficePage = () => {
  ...
  const handleDeleteFavorite = (officeId) => {
    if (window.confirm(t("favoriteOffices.confirmRemove"))) {
      dispatch(deleteFavoriteOffice(officeId));
    }
  };

  const handleClearAllFavorites = () => {
    if (window.confirm(t("favoriteOffices.confirmRemoveAll"))) {
      dispatch(deleteAllFavoriteOffices());
    }
  };
  ...
  return (
    <Container sx={{ mt: 4, pb: 6 }}>
      ...
      {favoriteOfficesError && (
        <Alert severity="error" sx={{ mt: 4 }}>
          {t("favoriteOffices.error")}{" "}
          {favoriteOfficesError?.message || t("favoriteOffices.unknownError")}
        </Alert>
      )}
          ...
          <Grid item xs={12} sm={6} md={4} key={favoriteItem.id}>
            ...
            >
              <CardMedia
                ...
                height="200"
                image={
                  favoriteItem.office?.office_photo?.url ||
              ...
              <CardActions sx={{ justifyContent: "space-between" }}>
                <Button
                  ...
                  onClick={() => handleViewOffice(favoriteItem.office.id)}
                >
                  {t("favoriteOffices.viewDetails")}
                </Button>
                <IconButton
                  aria-label={t("favoriteOffices.removeAria")}
                  onClick={() => handleDeleteFavorite(favoriteItem.office.id)}
                  disabled={favoriteOfficesLoading}
                  color="error"
                >
