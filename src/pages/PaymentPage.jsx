...
const PaymentPage = () => {
  ...
  const PaymentSuccessCard = () => (
    <Paper elevation={3} sx={{ p: 4, textAlign: "center" }}>
      ...
      <Typography variant="body2" gutterBottom>
        {t("payment.expirationMessage", {
          expires: reservation?.expires_at,
          type: t(`payment.typeOperation.${property?.typeOperation}`),
        })}
      </Typography>
      <Button
        variant="contained"
        color="primary"
        onClick={() => navigate("/reserved-property")}
        sx={{ mt: 3 }}
      >
        {t("payment.ok")}
      </Button>
  ...
  if (!property || !propertyId) {
    return (
      ...
      >
        <Typography variant="h5" color="text.primary" gutterBottom>
          {t("payment.noPropertyTitle")}
        </Typography>
        <Typography variant="body1" color="text.secondary">
          {t("payment.noPropertyMessage")}
        </Typography>
        <Button
          variant="contained"
          onClick={() => navigate("/")}
        ...
        {paymentStatus === "success" ? (
          ...
          <Paper elevation={6} sx={{ p: 4, borderRadius: 3 }}>
            ...
            <Typography
              ...
              mb={3}
            >
              {t("payment.formSubtitle", {
                name: property?.name || t("payment.defaultProperty"),
